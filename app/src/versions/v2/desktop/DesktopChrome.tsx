  ...
  return (
    <div className="flex h-full bg-ink text-text">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        {/* persistent deal strip */}
        {dealStage && <DealStrip stage={dealStage} />}
...
/* ---- deal strip ---------------------------------------------------------- */
function DealStrip({ stage }: { stage: DealSnapshot }) {
  return (
    <div className="flex items-center gap-4 border-b border-line bg-surface/50 px-6 py-2.5">
      {/* deal identity */}
      <div className="flex min-w-0 items-center gap-2">
        ...
        <div className="min-w-0 leading-tight">
          <p className="truncate text-xs font-extrabold">{deal.name}</p>
          <p className="truncate text-[10px] text-muted">{deal.summary}</p>
        </div>
      </div>
          ...
          {Array.from({ length: deal.totalStages }).map((_, i) => (
            <span
              ...
              className={`h-1.5 w-4 rounded-full ${
                i < stage.stageIndex ? 'bg-accent' : 'bg-line'
              }`}
            />
          ))}
        </div>
      ...
      <div className="ml-auto flex items-center gap-4">
        ...
        <div className="text-right leading-tight">
          ...
          <p className="flex items-center justify-end gap-1 text-sm font-extrabold text-accent">
      ...
      {/* downside callout */}
      {stage.downside && (
        <span className="hidden items-center gap-1.5 rounded-full bg-risk/10 px-3 py-1 text-[11px] font-bold text-risk lg:flex">
